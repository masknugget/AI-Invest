"""
Excel/JSON → LanceDB 导入脚本
用于将 xlsx 格式的问答数据导入向量数据库，支持语义搜索

Usage:
    python recommender/import_excel_to_lancedb.py --input data/faq.xlsx
    python recommender/import_excel_to_lancedb.py --input data/faq.xlsx --collection faq_v1 --clear
    python recommender/import_excel_to_lancedb.py --input data/faq.xlsx --dry-run
    python recommender/import_excel_to_lancedb.py --input data/faq.xlsx --test-query "如何开户"
"""
...
def build_documents(
    ...
    for i, record in enumerate(records):
        ...
        # 添加 meta_data（metadata 只支持 str/int/float/bool/None）
